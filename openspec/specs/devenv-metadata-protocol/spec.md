# devenv-metadata-protocol Specification

## Purpose

Defines the `--devenv-metadata` flag convention that allows scripts of any language to declare their parameter schema. When a script is invoked with `--devenv-metadata`, it prints a JSON array of parameter definitions to stdout and exits with code 0. DevEnv uses this to dynamically build the parameter input modal without needing language-specific comment parsers.
