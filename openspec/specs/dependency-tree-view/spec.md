...
- **THEN** each node SHALL show an infra icon, the service name, and its current status
...
#### Scenario: Shared dependencies appear at each reference
- **WHEN** both `frontend` and `backend` depend on `postgres`
- **THEN** `postgres` SHALL appear under each app that references it
...
### Requirement: Keyboard and mouse navigation for dependency tree
The system SHALL support keyboard and mouse navigation within the dependency tree.
...
- **THEN** `Enter` SHALL expand/collapse the selected node
- **THEN** `Escape` SHALL return focus to the main detail view

#### Scenario: Mouse click toggles expand/collapse
- **WHEN** the user clicks a tree node
- **THEN** the node SHALL be selected and focused
- **THEN** if the node is an expandable app node, it SHALL toggle expand/collapse
...
- **THEN** app nodes SHALL use Nerd Font cube icon and infra nodes SHALL use Nerd Font server icon
