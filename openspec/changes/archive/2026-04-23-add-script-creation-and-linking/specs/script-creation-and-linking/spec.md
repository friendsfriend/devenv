...
### Requirement: Create new script in config scripts directory
The system SHALL create a new script file under the config `scripts/` directory using the entered target name/path.
...
The system SHALL create a symlink in the config `scripts/` directory that maps the entered target name/path to the provided source script path.
