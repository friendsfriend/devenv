...
The system SHALL load script collections from the DevEnv config directory under `scripts/` and preserve the filesystem hierarchy for any folder depth, including symlink-backed script entries located within that tree.
...
- **WHEN** a script entry under `scripts/` is a symlink to an existing script file outside or inside the config tree
