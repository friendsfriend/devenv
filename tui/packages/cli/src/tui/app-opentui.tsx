...
import { ContentRouter, ModalOverlays, getTabName, getTabBorderColor, getHeaderSubtitle } from './views';
...
export function TUIApp(props: TUIAppProps) {
  ...
  // --- Actions ---
  const appActions = createAppActions(appStore, appDetailStore, uiStore, client, showError);
  const logActions = createLogActions(logStore, appStore, client, showError);
  const mrActions = createMrActions(appStore, mrStore, uiStore, client, showError);
  ...
  const providerActions = createProviderActions(appStore, providerStore, client, showError);
  const agentActions = createAgentActions(appStore, agentStore, client);
  const utilActions = createUtilActions(appStore, agentStore, uiStore, renderer, client);
  const pipelineActions = createPipelineActions(appStore, mrStore, client, showError);
  ...
  const getSelectedApp = (): App | undefined => appStore.filteredApps()[appStore.selectedIndex()];
  ...
  createEffect(on(logStore.showLogModal, (open) => {
    if (open) {
      const lastLine = Math.max(0, logStore.logs().split('\n').length - 1);
      logStore.setLogSelectedLine(lastLine);
      logStore.setLogScrollTop(Math.max(0, lastLine - logStore.logViewportHeight() + 1));
    }
  }, { defer: true }));
  ...
  // --- Initialization ---
  onMount(() => {
    void initializeApp({ client, appStore, appActions, showError, serverUrl: props.serverUrl });
  });
  ...
  // --- Keyboard dispatcher ---
  const kbStores: KeyboardStores = { appStore, logStore, mrStore, providerStore, uiStore, agentStore, appDetailStore };
  const kbActions: KeyboardActions = { appActions, logActions, mrActions, dockerActions, gitActions, providerActions, agentActions, utilActions, pipelineActions, helpActions };
  const kbCtx: KeyboardContext = { renderer, client, getSelectedApp, launchPi, getSelectableRows, showError };
  ...
  useKeyboard(async (event) => {
    ...
    if (await handleConnectProviderModalKeys(event, kbStores, kbActions)) return;
    ...
    if (await handleWorktreeManagerKeys(event, kbStores, kbActions, kbCtx)) return;
  ...
  // --- View props ---
  const viewStores: ViewStores = { appStore, logStore, mrStore, providerStore, uiStore, agentStore, appDetailStore };
  const viewActions: ViewActions = { appActions, logActions, mrActions, dockerActions, gitActions, providerActions, agentActions, utilActions, pipelineActions, helpActions };

  const headerSubtitleDeps = { appStore, mrStore, appDetailStore, helpActions, getSelectedApp };
  ...
  return (
    ...
    >
      <Layout
        header={<Header title="Development Environment" subtitle={getHeaderSubtitle(headerSubtitleDeps)} />}
        content={
          <ContentRouter
            stores={viewStores}
            actions={viewActions}
        ...
        footer={
          <StatusBar
            ...
            right={appStore.viewMode() === 'table' ? `Selected: ${appStore.selectedIndex() + 1}/${appStore.filteredApps().length}` : ''}
  ...
  try {
    ...
    const renderer = await createCliRenderer({
      ...
      consoleOptions: {
        ...
        keyBindings: [{ name: 'c', ctrl: true, shift: true, action: 'copy-selection' as const }],
        onCopySelection: (text: string) => {
          import('@devenv/core').then(({ copyToClipboard }) => {
