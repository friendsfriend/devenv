...
import type { App, InfraService, ScriptNode, ScriptVisibleRow, StatusLogEntry } from '@devenv/types';
...
function flattenScriptTree(nodes: ScriptNode[], expandedFolders: Set<string>, depth = 0): ScriptVisibleRow[] {
  ...
  for (const node of nodes) {
    ...
    if (node.nodeType === 'folder' && expandedFolders.has(node.relativePath) && node.children?.length) {
...
export function createAppStore() {
  ...
  const [previousViewMode, setPreviousViewMode] = createSignal<ViewMode | null>('table');
  const [tableSearchMode, setTableSearchMode] = createSignal(false);
  const [tableSearchQuery, setTableSearchQuery] = createSignal('');
  const [statusLogEntries, setStatusLogEntries] = createSignal<StatusLogEntry[]>([]);
  const [statusLogMaximized, setStatusLogMaximized] = createSignal(false);
  const [operationInProgressForApp, setOperationInProgressForApp] = createSignal<string | null>(null);
  ...
  const [expandedScriptFolders, setExpandedScriptFolders] = createSignal<Set<string>>(new Set());

  const scriptVisibleRows = createMemo(() => flattenScriptTree(scriptsTree(), expandedScriptFolders()));

  const scriptRowsAsApps = createMemo<App[]>(() => scriptVisibleRows().map((row): App => ({
    ...
    containerBaseName: '',
    resourceType: row.nodeType === 'folder' ? 'script-folder' as const : 'script-file' as const,
  ...
  const tableFilteredApps = createMemo(() => {
    ...
    return filteredApps().filter((app) =>
      Object.values(app).some((v) => v != null && String(v).toLowerCase().includes(q)),
    );
  });
  ...
  const tableTabs = createMemo((): TableTab<TabType>[] => {
    const allApps = apps();
    return [
      { id: 'applications', label: 'Applications', count: allApps.filter((app) => app.appType === 'APP').length },
      { id: 'infrastructure', label: 'Infrastructure', count: infraServices().length },
      { id: 'libraries', label: 'Libraries', count: allApps.filter((app) => app.appType === 'LIB').length },
