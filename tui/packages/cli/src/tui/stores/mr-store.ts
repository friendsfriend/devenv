...
import type { Discussion, Job, MergeRequest, MRChange, TestCase, TestSummary } from '@devenv/types';
...
export function createMrStore() {
  ...
  const [mrTestSummary, setMrTestSummary] = createSignal<TestSummary | null>(null);
  ...
  const [mrJobsForDetailLoading, setMrJobsForDetailLoading] = createSignal(false);
  ...
  const [currentPipelineId, setCurrentPipelineId] = createSignal<number | null>(null);
  ...
  const [testDetailCopyStatus, setTestDetailCopyStatus] = createSignal<string | null>(null);
  const [selectedChangedFileIndex, setSelectedChangedFileIndex] = createSignal(0);
  ...
  const [currentDiffFile, setCurrentDiffFile] = createSignal<MRChange | null>(null);
  ...
  const [diffModalForceSplitView, setDiffModalForceSplitView] = createSignal<boolean>(false);
  ...
  const [collapsedThreads, setCollapsedThreads] = createSignal<Set<string>>(new Set());
  const [discussionsShowOnlyComments, setDiscussionsShowOnlyComments] = createSignal(false);
  const [mrSearchMode, setMrSearchMode] = createSignal(false);
  const [mrSearchQuery, setMrSearchQuery] = createSignal('');
  const [changedFilesSearchMode, setChangedFilesSearchMode] = createSignal(false);
  const [changedFilesSearchQuery, setChangedFilesSearchQuery] = createSignal('');
  ...
  const changedFilesFiltered = createMemo(() => {
    ...
    return mrChanges().filter((c) => [c.new_path, c.old_path].some((v) => v && v.toLowerCase().includes(q)));
  });
  ...
  return {
    ...
    mrAiVisible, setMrAiVisible,
    mrAiLoading, setMrAiLoading,
    mrAiStreaming, setMrAiStreaming,
    mrAiSummary, setMrAiSummary,
    mrAiError, setMrAiError,
    mrAiFollowupText, setMrAiFollowupText,
    mrAiPostingComments, setMrAiPostingComments,
    mrAiCommentsPosted, setMrAiCommentsPosted,
    get mrAiScrollBoxRef() { return mrAiScrollBoxRef; },
    set mrAiScrollBoxRef(v: import('@opentui/core').ScrollBoxRenderable | undefined) { mrAiScrollBoxRef = v; },
    get mrAiAtBottom() { return mrAiAtBottom; },
    set mrAiAtBottom(v: boolean) { mrAiAtBottom = v; },
    get mrAiLastScrollTop() { return mrAiLastScrollTop; },
    set mrAiLastScrollTop(v: number) { mrAiLastScrollTop = v; },
  };
}
