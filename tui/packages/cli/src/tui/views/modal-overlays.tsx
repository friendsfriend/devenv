...
export function ModalOverlays(props: ModalOverlaysProps) {
  const { appStore, logStore, mrStore, providerStore, uiStore, agentStore } = props.stores;
  ...
  return (
    <>
      ...
      <Show when={uiStore.showPassphraseModal() && uiStore.pendingSshHost()?.identityFile}>
        <PassphraseModal
        ...
        <WorktreeManagerModal
          appName={
            uiStore.worktreeManagerAppId()
              ? (appStore.apps().find((a) => a.ident === uiStore.worktreeManagerAppId())?.displayName ?? uiStore.worktreeManagerAppId() ?? '')
              : ''
          }
        ...
        <ProfilePickerView
          ...
          onSubmit={(profile) => {
            ...
            if (ident) {
              ...
              if (appOrSvc) {
                void dockerActions.performDockerOperation('start', appOrSvc as any, profile === 'default (no profile)' ? '' : profile);
              }
            }
          }}
      ...
      <Show when={uiStore.showScriptArgsModal()}>
        <ScriptArgsModal
          scriptName={appStore.tableFilteredApps()[appStore.selectedIndex()]?.displayName || ''}
...
<Show when={uiStore.showLoadingModal()}>
        ...
        >
          ...
          >
            <text fg={uiColors.primary}>{props.spinnerFrames[appStore.spinnerFrame()]}</text>
            <text fg={uiColors.textPrimary}>{uiStore.loadingModalMessage()}</text>
          </box>
        </box>
      </Show>
      ...
      <Show when={mrStore.showDiffModal() && mrStore.currentDiffFile()}>
        <DiffViewModal
          filePath={mrStore.currentDiffFile()!.new_path || mrStore.currentDiffFile()!.old_path}
          ...
          onNavigateFile={(direction) => {
            ...
            let newIndex: number;
            if (direction === 1) {
              newIndex = (mrStore.selectedChangedFileIndex() + 1) % changes.length;
            } else {
      ...
      <Show when={logStore.showLogModal()}>
        <LogModal
          ...
          onAiScrollBoxReady={(sb) => { logStore.logAiScrollBoxRef = sb; }}
        />
      </Show>
      ...
      <Show when={mrStore.mrAiVisible()}>
        <MrAiReviewOverlay
          ...
          onScrollBoxReady={(sb) => { mrStore.mrAiScrollBoxRef = sb; }}
        />
      </Show>

      <Show when={mrStore.showTestDetailModal() && mrStore.selectedTestForDetail()}>
        <TestDetailModal
      ...
      <Show when={uiStore.copyStatus()}>
        ...
        >
          <text fg={uiColors.bgBase} attributes={TextAttributes.BOLD}>{uiStore.copyStatus()}</text>
        </box>
      </Show>
    </>
  );
