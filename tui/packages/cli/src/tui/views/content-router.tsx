...
export function ContentRouter(props: ContentRouterProps) {
  const { appStore, mrStore, providerStore, appDetailStore } = props.stores;
  const { helpActions, pipelineActions, logActions, mrActions } = props.actions;
  const tableColumns = () => appStore.activeTab() === 'scripts'
    ? props.scriptColumns
    : appStore.activeTab() === 'infrastructure'
      ? props.columns.filter((column) => column.key !== 'branch' && column.key !== 'gitStatus')
      : props.columns;
            ...
            <Show
              ...
              fallback={
                <Show
                  ...
                  fallback={
                    <Show
                      when={appStore.viewMode() === 'mergeRequestDetail' && mrStore.selectedMR()}
                      fallback={
                        <Show
                          when={appStore.viewMode() === 'changedFiles'}
                          fallback={
                            <Show
                              when={appStore.viewMode() === 'discussionsView'}
                              fallback={
                                <Show
                                  when={appStore.viewMode() === 'testResults'}
                                  fallback={
                                    <Show
                                      when={appStore.viewMode() === 'jobs'}
                                      fallback={
                                        <Show
                                          when={appStore.viewMode() === 'appDetail' && appDetailStore.appDetailApp()}
                                          fallback={
                                            <Show
                                              when={appStore.viewMode() === 'providers'}
                                              fallback={
                                        ...
                                        >
                                          <AppDetailView
                                            app={appDetailStore.appDetailApp()!}
                                    ...
                                    >
                                      <JobsDetailView
                                        jobs={mrStore.jobs()}
                                        pipelineId={mrStore.currentPipelineId() || 0}
                                        ...
                                        onClose={pipelineActions.backToMRDetail}
                                        onViewJobLogs={logActions.loadJobLogs}
                                        onRetryJob={pipelineActions.retryJob}
                                        onCancelJob={pipelineActions.cancelJob}
                    ...
                    >
                      <MergeRequestDetailView
                        ...
                        testSummary={mrStore.mrTestSummary() || undefined}
