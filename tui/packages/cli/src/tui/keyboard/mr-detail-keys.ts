...
export async function handleMrDetailKeys(
  ...
  getLogger().write('DEBUG', `[MR DETAIL] Key: name="${event.name}", sequence="${event.sequence}", shift=${event.shift}, ctrl=${event.ctrl}`);
  ...
  if (mrStore.mrAiVisible()) {
    ...
    // Dismiss
    const isEsc = name === 'escape' || name === 'Escape' || name === 'esc'
      || event.sequence === '\x1b' || event.raw === '\x1b';
    if (isEsc) {
    ...
    if (mrStore.mrAiSummary() !== null && mrStore.mrAiScrollBoxRef) {
      ...
      if (event.ctrl && name === 'j') { mrStore.mrAiAtBottom = false; sb.scrollBy(1);  mrStore.mrAiLastScrollTop = sb.scrollTop;  return true; }
      if (event.ctrl && name === 'k') { mrStore.mrAiAtBottom = false; sb.scrollBy(-1); mrStore.mrAiLastScrollTop = sb.scrollTop;  return true; }
      if (event.ctrl && name === 'g') { mrStore.mrAiAtBottom = false; sb.scrollTo(0);  mrStore.mrAiLastScrollTop = 0;             return true; }
      if (event.ctrl && name === 'G') { mrStore.mrAiAtBottom = true;  sb.scrollTo(sb.scrollHeight); mrStore.mrAiLastScrollTop = sb.scrollTop; return true; }
    }

    // Enter — close completed review
    if (name === 'return' || name === 'Return' || name === 'enter' || name === 'Enter') {
      if (mrStore.mrAiSummary() !== null && !mrStore.mrAiLoading() && !mrStore.mrAiStreaming()) {
        mrStore.setMrAiVisible(false);
      }
      return true;
    }
  ...
  if (event.name === 'q' || event.name === 'Q') { appActions.exitApp(); return true; }
  if (event.name === '?' || event.sequence === '?') { helpActions.showHelp(); return true; }

  if (event.sequence === 'C') {
    const changes = mrStore.mrChanges();
    if (changes && changes.length > 0) { mrStore.setSelectedChangedFileIndex(0); appStore.setViewMode('changedFiles'); }
    return true;
  }
  ...
  if (event.sequence === 'D') {
    getLogger().write('DEBUG', `Shift+D pressed! event.sequence="${event.sequence}"`);
    const discussions = mrStore.mrDiscussions();
    getLogger().write('DEBUG', `Discussions count: ${discussions?.length || 0}`);
    try {
      if (Array.isArray(discussions) && discussions.length > 0) {
        ...
        getLogger().write('INFO', `Switched to discussionsView mode with ${discussions.length} discussions`);
      }
    } catch (e) {
  ...
  if (event.name === 'r') { await mrActions.rebaseMR(); return true; }

  if (
    event.name === 'escape' || event.name === 'Escape' || event.name === 'esc' ||
    event.sequence === '\x1b' || event.raw === '\x1b'
  ) {
