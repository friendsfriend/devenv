...
export async function handleTableKeys(
  ...
  const { appActions, logActions, mrActions, dockerActions, gitActions, providerActions, agentActions, utilActions, helpActions } = actions;
  ...
  if (uiStore.showBranchSelector()) {
    ...
    if (event.name === 'down' || event.name === 'Down' || (event.ctrl && event.name === 'j')) {
      uiStore.setBranchSelectorIndex((prev) => Math.min(prev + 1, uiStore.filteredBranches().length - 1));
      return true;
    }
    if (event.name === 'up' || event.name === 'Up' || (event.ctrl && event.name === 'k')) {
    ...
    if (event.ctrl && event.name === 'n' && !uiStore.branchSelectorWorktreeCreateMode()) {
      uiStore.setCreateBranchName('');
    ...
    if (event.name === 'j') {
      uiStore.setBranchSelectorIndex((prev) => Math.min(prev + 1, branchMaxIdx));
      return true;
    }
    if (event.name === 'k') {
      uiStore.setBranchSelectorIndex((prev) => Math.max(prev - 1, 0));
    ...
    if (event.name === 'd') {
      uiStore.setBranchSelectorIndex((prev) => Math.min(prev + Math.floor(branchPageSize / 2), branchMaxIdx));
      return true;
    }
    if (event.name === 'u') {
      uiStore.setBranchSelectorIndex((prev) => Math.max(prev - Math.floor(branchPageSize / 2), 0));
      return true;
    }
    ...
    if (!uiStore.branchSelectorWorktreeCreateMode()) {
      ...
      if (event.sequence === 'l') {
        const selectedBranch = uiStore.filteredBranches()[uiStore.branchSelectorIndex()];
        if (selectedBranch) utilActions.launchLazygitBranchLog(selectedBranch.name);
        return true;
      }
      // f — git fetch
      if (event.sequence === 'f') {
  ...
  if (appStore.viewMode() === 'appDetail') {
    if (event.name === 'escape' || event.name === 'esc' || event.sequence === '\x1b') {
  ...
  const isLetter = event.sequence && event.sequence.length === 1 && /[a-zA-Z]/.test(event.sequence);
  ...
  switch (key) {
    ...
    case 'j':
      if (appList.length > 0) {
        appStore.setSelectedIndex((prev) => Math.min(prev + 1, appList.length - 1));
      }
      break;
    case 'up':
    case 'Up':
    case 'k':
      ...
      break;
    case 'l':
      if (appStore.activeTab() !== 'scripts' && appList.length > 0) logActions.loadContainerLogs();
      break;
    case 'L':
      ...
      break;
    case 'o':
      if (appStore.activeTab() !== 'scripts' && appList.length > 0) logActions.loadOperationLogs();
      break;
    case 'm':
      // Show MR detail for current branch (lowercase m)
      if (appStore.activeTab() !== 'scripts' && appList.length > 0) mrActions.loadMergeRequestForCurrentBranch();
      break;
    case 'M':
      // Show all open MRs list (uppercase M / Shift+M)
      if (appStore.activeTab() !== 'scripts' && appList.length > 0) mrActions.loadAllMergeRequests();
      break;
    case 'c':
    ...
    case 's':
      ...
      if (appList.length > 0) {
        ...
        client.getProfiles(app.ident).then((result) => {
    ...
    case 'S':
      ...
      if (appStore.activeTab() !== 'scripts' && appList.length > 0) dockerActions.requestDockerOperation('stop');
      break;
    case 'r':
    ...
    case 'R':
      ...
      if (appStore.activeTab() !== 'scripts' && appList.length > 0) dockerActions.requestDockerOperation('restart');
      break;
    case 'f':
      // Git fetch (lowercase f)
      if (appStore.activeTab() !== 'scripts' && appList.length > 0) gitActions.performGitFetch();
      break;
    case 'g':
      // Branch selector (lowercase g)
      if (appStore.activeTab() !== 'scripts' && appList.length > 0) gitActions.openBranchSelector();
      break;
    case 'w': {
      ...
      if (appStore.activeTab() !== 'scripts' && appList.length > 0) {
        ...
        if (app) {
          ...
          client.listWorktrees(app.ident).then((wts) => {
            uiStore.setWorktreeManagerWorktrees(wts);
            // Re-clamp the index in case the user pressed j/k before the list arrived
            uiStore.setWorktreeManagerSelectedIndex((prev) => Math.min(Math.max(prev, 0), Math.max(0, wts.length - 1)));
          }).catch((e) => {
            uiStore.setShowWorktreeManagerModal(false);
            uiStore.setWorktreeManagerAppId(null);
            showError('Failed to Load Worktrees', `Could not load worktrees for ${app.displayName}.\n\nError: ${e instanceof Error ? e.message : 'Unknown error'}`);
          });
        }
      }
      break;
    ...
    case 'G':
      ...
      if (appStore.activeTab() !== 'scripts' && appList.length > 0) utilActions.launchLazygit();
      break;
    case 'd':
      // Open lazydocker for selected app
      if (appStore.activeTab() !== 'scripts' && appList.length > 0) utilActions.launchLazydocker();
      break;
    case 'B':
      // Build (uppercase B)
      if (appStore.activeTab() !== 'scripts' && appList.length > 0) dockerActions.performBuild();
      break;
    case 'A':
    ...
    case 'H':
      ...
      break;
    case '+':
      if (appStore.activeTab() === 'scripts') void appActions.openAddScriptModal();
