...
export function createMrActions(
  ...
  const loadMergeRequestForCurrentBranch = async () => {
    ...
    try {
      const result = await client.getMergeRequests(app.ident, 'opened', 'current', app.sourceType, 1, mrStore.perPage());
      if (result.items.length > 0) {
        ...
        const allResult = await client.getMergeRequests(app.ident, 'opened', 'all', app.sourceType, 1, mrStore.perPage());
  ...
  const loadAllMergeRequests = async (page?: number, search?: string) => {
    ...
    appStore.setViewMode('mergeRequests');
    try {
      const result = await client.getMergeRequests(app.ident, 'opened', 'all', app.sourceType, p, mrStore.perPage(), search);
      mrStore.setMergeRequests(result.items);
      mrStore.setTotalPages(result.totalPages);
      mrStore.setTotalCount(result.totalCount);
      mrStore.setCurrentPage(result.currentPage);
  ...
  const showMRDetail = (mr: NonNullable<ReturnType<typeof mrStore.selectedMR>>) => {
  ...
  const loadMRDetailData = async (mr: NonNullable<ReturnType<typeof mrStore.selectedMR>>) => {
    const app = getSelectedApp();
    if (!app) return;
    const changesPromise = (async () => {
      try { mrStore.setMrChanges(await client.getMRChanges(app.ident, mr.iid, app.sourceType)); }
      catch (e) { mrStore.setMrChangesError(errMsg(e)); }
      finally { mrStore.setMrChangesLoading(false); }
    })();
    const jobsPromise = (async () => {
      if (!mr.head_pipeline) return;
      try { mrStore.setMrJobsForDetail(await client.getPipelineJobs(app.ident, mr.head_pipeline.id, app.sourceType)); }
      catch (e) { mrStore.setMrJobsForDetailError(errMsg(e)); }
      finally { mrStore.setMrJobsForDetailLoading(false); }
    })();
    const testSummaryPromise = (async () => {
      if (!mr.head_pipeline) return;
      try { mrStore.setMrTestSummary(await client.getTestSummary(app.ident, mr.head_pipeline.id, app.sourceType)); }
      catch (e) { mrStore.setMrTestError(errMsg(e)); }
      finally { mrStore.setMrTestLoading(false); }
    })();
    const discussionsPromise = (async () => {
      try { mrStore.setMrDiscussions(await client.getMRDiscussions(app.ident, mr.iid, app.sourceType)); }
      catch (e) { mrStore.setMrDiscussionsError(errMsg(e)); }
      finally { mrStore.setMrDiscussionsLoading(false); }
    })();
    await Promise.all([changesPromise, jobsPromise, testSummaryPromise, discussionsPromise]);
  };
  ...
  const submitComment = async () => {
    ...
    if (!comment) return showError('Empty Comment', 'Please enter a comment before submitting');
    if (!app || !mr || !diffFile) return showError('Invalid State', 'Missing required data for creating comment');
    mrStore.setCommentSubmitting(true);
    try {
      const versions = await client.getMRVersions(app.ident, mr.iid);
      if (!versions.length) return showError('No Versions', 'Could not fetch MR versions');
      ...
      const position = {
        ...
        newLine: diffFile.diff_lines?.[mrStore.diffModalSelectedLine()]?.new_line,
        oldLine: diffFile.diff_lines?.[mrStore.diffModalSelectedLine()]?.old_line,
      };
      await client.createMRComment(app.ident, mr.iid, comment, position);
      showError('Comment Posted', 'Your comment was successfully added to the merge request');
  ...
  const replyToDiscussion = async (discussionID: string, body: string) => {
    const app = appStore.filteredApps()[appStore.selectedIndex()];
    const mr = mrStore.selectedMR();
    if (!app || !mr) return showError('Invalid State', 'Missing required data for replying to discussion');
    if (!body.trim()) return showError('Empty Reply', 'Please enter a reply before submitting');
    try {
      await client.replyToDiscussion(app.ident, mr.iid, discussionID, body);
      mrStore.setMrDiscussions(await client.getMRDiscussions(app.ident, mr.iid, app.sourceType));
  ...
  const resolveDiscussion = async (discussionID: string, resolveAction: 'resolve' | 'unresolve') => {
    const app = appStore.filteredApps()[appStore.selectedIndex()];
    const mr = mrStore.selectedMR();
    if (!app || !mr) return showError('Invalid State', 'Missing required data for resolving discussion');
    try {
      await client.resolveDiscussion(app.ident, mr.iid, discussionID, resolveAction);
      mrStore.setMrDiscussions(await client.getMRDiscussions(app.ident, mr.iid, app.sourceType));
    } catch (e) {
  ...
  const toggleMRApproval = async () => {
    ...
    try {
      ...
      const fetched = await client.getMergeRequests(app.ident, 'opened', 'all', app.sourceType, mrStore.currentPage(), mrStore.perPage());
      const updated = fetched.items.find((m) => m.iid === mr.iid);
      if (updated) {
        mrStore.setSelectedMR(updated);
        mrStore.setMergeRequests((mrs) => mrs.map((m) => (m.iid === updated.iid ? updated : m)));
      }
  ...
  const rebaseMR = async () => {
    ...
    try {
      ...
      if (updated) {
        mrStore.setSelectedMR(updated);
        mrStore.setMergeRequests((mrs) => mrs.map((m) => (m.iid === updated.iid ? updated : m)));
        if (updated.merge_error) showError('Rebase Failed', updated.merge_error);
        else if (updated.rebase_in_progress) showError('Rebase In Progress', 'Rebase operation has been queued and is currently in progress. Please wait for it to complete.');
        else showError('Rebase Successful', 'Merge request has been rebased successfully.');
      }
      await loadMRDetailData(updated || mr);
    } catch (e) {
      showError('Rebase Failed', errMsg(e));
      getLogger().write('ERROR', `Failed to rebase MR: ${e instanceof Error ? e.message : String(e)}`);
    } finally {
      uiStore.setShowLoadingModal(false);
    }
  };
  ...
  const getDiscussionAtCurrentLine = () => {
    ...
    if (!diffFile || !diffFile.diff_lines || !mrStore.mrDiscussions()) return null;
    ...
    for (const discussion of mrStore.mrDiscussions()) {
      ...
      const matchesFile = position.new_path === diffFile.new_path || position.old_path === diffFile.old_path;
      if (!matchesFile) continue;
      if (diffLine.type === 'new' && position.new_line === diffLine.new_line) return discussion;
      if (diffLine.type === 'old' && position.old_line === diffLine.old_line) return discussion;
      if (diffLine.type === 'match' && (position.new_line === diffLine.new_line || position.old_line === diffLine.old_line)) return discussion;
    }
    return null;
  };
  ...
  const getLinesWithComments = (): number[] => {
    ...
    const hasLineCommentFn = (
      ...
      const matchesNewLine = lt === 'added' && curNew && position.new_line === curNew;
      const matchesOldLine = lt === 'removed' && curOld && position.old_line === curOld;
      const matchesContext = lt === 'context' && (position.new_line === curNew || position.old_line === curOld);
    ...
    for (const line of diffLines) {
      ...
      const hasComment = discussions.some((d) => hasLineCommentFn(d, lineType, currentOldLine, currentNewLine));
    ...
    try {
      ...
      for await (const delta of client.analyzeMRWithAIStream(
        ...
        if (mrStore.mrAiAtBottom && mrStore.mrAiScrollBoxRef) {
          if (mrStore.mrAiScrollBoxRef.scrollTop !== mrStore.mrAiLastScrollTop) {
            mrStore.mrAiAtBottom = false;
          } else {
            mrStore.mrAiScrollBoxRef.scrollTo(mrStore.mrAiScrollBoxRef.scrollHeight);
  ...
  const postMrAiComments = async () => {
    ...
    if (app.sourceType === 'github') {
      showError('Not Supported', 'Posting AI review comments is only supported for GitLab MRs.');
      return;
    }
